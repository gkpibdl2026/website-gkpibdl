...
## 🎵 Fitur Baru: Song Module & Bible Data
...
- **Logo Replacement**: Mengganti logo Vercel default dengan Logo GKPI resmi pada:
  - Admin Sidebar
  - Public Navbar
  - Favicon (`layout.tsx` metadata)
- **Refinement**: Mengubah strategi load logo dari External URL (Wikimedia) menjadi Local Asset (`public/logo-gkpi.png`) untuk menghindari masalah 403/404 dan caching yang tidak konsisten.
- **Cleanup**: Menghapus `src/app/favicon.ico` bawaan Next.js yang menimpa konfigurasi metadata.

### 7. Bug Fixes & Refinements

- **Build Fixes**: Memperbaiki error TypeScript pada `TataIbadahEditor.tsx` dan `ModuleRenderer.tsx` terkait properti `SongVersesSelector` (`selectedVerses` vs `selectedSections`).
- **Cleanup**: Membersihkan script temporary (`debug-*.js`, `check-*.js`, `fix-toba.js`) dari folder `scripts/` agar lebih rapi.
...
### ⚠️ Required Action: Database Migrations

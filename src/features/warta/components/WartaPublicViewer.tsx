...
export default function WartaPublicViewer({ warta }: Props) {
        ...
        <div className="space-y-3">
          {filteredVerses.map((verse) => (
            <div key={verse.verse} className="flex gap-3">
              <span className="font-bold text-gray-500 w-6 flex-shrink-0">{verse.verse}.</span>
  ...
  const renderModule = (module: WartaModule, index: number) => {
    switch (module.type) {
      ...
      case 'AYAT': {
        ...
        return (
          <div key={module.id} className="bg-emerald-50 dark:bg-emerald-900/20 rounded-xl p-4 md:p-6 border border-emerald-100 dark:border-emerald-800">
            <div className="flex items-start gap-3">
              <span className="flex-shrink-0 w-7 h-7 rounded-full bg-emerald-600 dark:bg-emerald-500 text-white flex items-center justify-center text-sm font-bold">
                {index + 1}
              </span>
              ...
              {items.map((item, idx) => {
                ...
                return (
                  <li key={item.id} className="flex gap-3">
                    <span className="font-bold text-purple-600 dark:text-purple-400 w-6 flex-shrink-0">
                      {idx + 1}.
                    </span>
      ...
      case 'PELAYAN_IBADAH': {
        const data = module.data as { items: PelayanItem[] }
        const items = data.items || []
        return (
          <div key={module.id} className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4 md:p-6 border border-amber-100 dark:border-amber-800">
            <div className="flex items-start gap-3 mb-4">
              <span className="flex-shrink-0 w-7 h-7 rounded-full bg-amber-600 dark:bg-amber-500 text-white flex items-center justify-center text-sm font-bold">
                {index + 1}
              </span>
            ...
            <div className="space-y-2">
              {items.map((item) => (
                <div key={item.id} className={`flex justify-between ${sizes.base}`}>
                  <span className="text-amber-700 dark:text-amber-300">{item.role}</span>
      ...
      case 'PENGUMUMAN': {
        ...
        return (
          <div key={module.id} className="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-4 md:p-6 border border-yellow-100 dark:border-yellow-800">
            <div className="flex items-start gap-3 mb-4">
              <span className="flex-shrink-0 w-7 h-7 rounded-full bg-yellow-600 dark:bg-yellow-500 text-white flex items-center justify-center text-sm font-bold">
                {index + 1}
              </span>
      ...
      case 'STATISTIK': {
        ...
        return (
          <div key={module.id} className="bg-cyan-50 dark:bg-cyan-900/20 rounded-xl p-4 md:p-6 border border-cyan-100 dark:border-cyan-800">
            <div className="flex items-start gap-3 mb-4">
              <span className="flex-shrink-0 w-7 h-7 rounded-full bg-cyan-600 dark:bg-cyan-500 text-white flex items-center justify-center text-sm font-bold">
                {index + 1}
              </span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {data.dewasa !== undefined && (
                <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-lg">
                  <div className={`font-bold text-cyan-600 dark:text-cyan-400 ${sizes.heading}`}>{data.dewasa}</div>
                  <div className="text-sm text-cyan-700 dark:text-cyan-300">Dewasa</div>
                </div>
              )}
              {data.pemuda !== undefined && (
                <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-lg">
                  <div className={`font-bold text-cyan-600 dark:text-cyan-400 ${sizes.heading}`}>{data.pemuda}</div>
                  <div className="text-sm text-cyan-700 dark:text-cyan-300">Pemuda</div>
                </div>
              )}
              {data.remaja !== undefined && (
                <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-lg">
                  <div className={`font-bold text-cyan-600 dark:text-cyan-400 ${sizes.heading}`}>{data.remaja}</div>
                  <div className="text-sm text-cyan-700 dark:text-cyan-300">Remaja</div>
                </div>
              )}
              {data.anakAnak !== undefined && (
                <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-lg">
                  <div className={`font-bold text-cyan-600 dark:text-cyan-400 ${sizes.heading}`}>{data.anakAnak}</div>
                  <div className="text-sm text-cyan-700 dark:text-cyan-300">Anak-anak</div>
                </div>
              )}
            </div>
            {data.total !== undefined && (
              <div className={`mt-4 text-center font-bold text-cyan-900 dark:text-cyan-100 ${sizes.subheading}`}>
                Total: {data.total} jemaat
              </div>
            )}
          </div>
        )
      ...
      case 'KEUANGAN': {
        ...
        return (
          <div key={module.id} className="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 md:p-6 border border-green-100 dark:border-green-800">
            <div className="flex items-start gap-3 mb-4">
              <span className="flex-shrink-0 w-7 h-7 rounded-full bg-green-600 dark:bg-green-500 text-white flex items-center justify-center text-sm font-bold">
                {index + 1}
              </span>
      ...
      case 'ULANG_TAHUN': {
        const data = module.data as { items: UlangTahunItem[] }
        const items = data.items || []
        return (
          <div key={module.id} className="bg-pink-50 dark:bg-pink-900/20 rounded-xl p-4 md:p-6 border border-pink-100 dark:border-pink-800">
            <div className="flex items-start gap-3 mb-4">
              <span className="flex-shrink-0 w-7 h-7 rounded-full bg-pink-600 dark:bg-pink-500 text-white flex items-center justify-center text-sm font-bold">
                {index + 1}
              </span>
            ...
            <div className="space-y-2">
              {items.map((item) => (
                <div key={item.id} className={`flex justify-between ${sizes.base}`}>
                  <span className="text-pink-900 dark:text-pink-100">{item.name}</span>
                  <span className="text-pink-600 dark:text-pink-400">{item.date}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      case 'JEMAAT_SAKIT': {
        ...
        return (
          <div key={module.id} className="bg-red-50 dark:bg-red-900/20 rounded-xl p-4 md:p-6 border border-red-100 dark:border-red-800">
            <div className="flex items-start gap-3 mb-4">
              <span className="flex-shrink-0 w-7 h-7 rounded-full bg-red-600 dark:bg-red-500 text-white flex items-center justify-center text-sm font-bold">
                {index + 1}
              </span>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      {/* Header */}
      <header className="bg-gradient-to-br from-blue-600 to-purple-700 text-white py-8 md:py-12">
      ...
      <div className="fixed bottom-4 right-4 z-50 flex items-center gap-1 bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-700 rounded-full p-1">
        ...
        <span className="text-xs font-medium text-gray-500 dark:text-gray-400 px-2 min-w-[32px] text-center">
